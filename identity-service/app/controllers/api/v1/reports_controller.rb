class Api::V1::ReportsController < ApplicationController
    ...
    begin
      ...
      attachments = @report.attachments.map do |attachment|
...

        {
          ...
          url: "#{ENV['IDENTITY_SERVICE_URL']}#{Rails.application.routes.url_helpers.rails_blob_url(attachment, only_path: true)}"
        }
      end
  ...
  def validate_report_files(report, params)
    ...
    current_urls = current_attachments.map { |a| Rails.application.routes.url_helpers.rails_blob_url(a, host: ENV['IDENTITY_SERVICE_URL']) }
  ...
  def handle_report_files_update(report)
    ...
    attachments_to_delete = current_attachments.select do |attachment|
      url = Rails.application.routes.url_helpers.rails_blob_url(attachment, host: ENV['IDENTITY_SERVICE_URL'])
