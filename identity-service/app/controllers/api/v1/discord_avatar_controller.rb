    ...
    class DiscordAvatarController < ApplicationController
      ...
      def generate_avatar_url(avatar)
        if avatar.file.attached?
          host_uri = URI.parse(ENV["IDENTITY_SERVICE_URL"])
      ...
      def normalize_domain(url)
        ...
        auth_service_url = ENV['IDENTITY_SERVICE_URL']
        unless auth_service_url.present?
          Rails.logger.warn "IDENTITY_SERVICE_URL is not set, cannot normalize domain"
          return url
        end
      ...
      def avatar_json(avatar)
        {
          ...
          url: avatar.file.attached? ? normalize_domain(rails_blob_url(avatar.file, host: ENV["IDENTITY_SERVICE_URL"])) : nil,
