...
RSpec.describe UserTikTokUnbindConsumer do
    ...
    context "when user exists" do
      let!(:user) do
        create(:user).tap do |u|
          u.update_columns(
            tiktok_channel_name: "MyTikTokChannel",
            tiktok_url: "https://tiktok.com/@mychannel"
          )
        end
      end

      it "clears tiktok_channel_name and tiktok_url" do
        ...
        user.reload
        expect(user.tiktok_channel_name).to be_nil
        expect(user.tiktok_url).to be_nil
      end
      ...
      it "handles payload that is a JSON string gracefully (does not clear fields)" do
        ...
        user.reload
        expect(user.tiktok_channel_name).to eq("MyTikTokChannel")
        expect(user.tiktok_url).to eq("https://tiktok.com/@mychannel")
      end

      it "processes multiple messages in a batch" do
        user2 = create(:user).tap do |u|
          u.update_columns(
            tiktok_channel_name: "SecondTikTok",
            tiktok_url: "https://tiktok.com/@second"
          )
        end
        ...
        expect(user.reload.tiktok_channel_name).to be_nil
        expect(user2.reload.tiktok_channel_name).to be_nil
      end
    end
