...
RSpec.describe UserYoutubeUnbindConsumer do
    ...
    context "when user exists" do
      let!(:user) do
        create(:user).tap do |u|
          u.update_columns(
            youtube_channel_name: "MyYoutubeChannel",
            youtube_url: "https://youtube.com/c/mychannel"
          )
        end
      end

      it "clears youtube_channel_name and youtube_url" do
        ...
        user.reload
        expect(user.youtube_channel_name).to be_nil
        expect(user.youtube_url).to be_nil
      end
      ...
      it "handles payload that is a JSON string gracefully (does not clear fields)" do
        ...
        user.reload
        expect(user.youtube_channel_name).to eq("MyYoutubeChannel")
        expect(user.youtube_url).to eq("https://youtube.com/c/mychannel")
      end

      it "processes multiple messages in a batch" do
        user2 = create(:user).tap do |u|
          u.update_columns(
            youtube_channel_name: "SecondYoutube",
            youtube_url: "https://youtube.com/c/second"
          )
        end
        ...
        expect(user2.reload.youtube_channel_name).to be_nil
      end
    end
