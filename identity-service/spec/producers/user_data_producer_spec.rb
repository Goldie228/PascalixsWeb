...
RSpec.describe UserDataProducer do
  ...
  describe ".publish" do
    ...
    context "when user data has not changed (deduplication)" do
      before do
        # IDENTITY_SERVICE_URL не задан — normalize_domain возвращает URL как есть
        ENV.delete("IDENTITY_SERVICE_URL")
      end
    ...
    context "when discord avatar URL needs normalization" do
      before do
        ENV["IDENTITY_SERVICE_URL"] = "https://auth.example.com"
      ...
      it "normalizes avatar domain to IDENTITY_SERVICE_URL" do
        described_class.publish(user)
    ...
    context "when avatar domain already matches IDENTITY_SERVICE_URL" do
      before do
        ENV["IDENTITY_SERVICE_URL"] = "https://auth.example.com"
    ...
    context "when IDENTITY_SERVICE_URL is not set" do
      before do
        ENV.delete("IDENTITY_SERVICE_URL")
      end

      it "returns avatar URL as-is and logs a warning" do
        expect(Rails.logger).to receive(:warn).with("IDENTITY_SERVICE_URL is not set, cannot normalize domain")
