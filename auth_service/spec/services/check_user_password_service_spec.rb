...
RSpec.describe CheckUserPasswordService do
  describe ".call" do
    ...
    let(:api_url) { "#{ENV['MINECRAFT_SERVICE_URL']}/api/v1/players/#{minecraft_account.nickname}/check_password" }

    before do
      # Устанавливаем ENV напрямую для сервиса
      @original_minecraft_url = ENV["MINECRAFT_SERVICE_URL"]
      @original_api_key = ENV["INTER_SERVICE_API_KEY"]
      ENV["MINECRAFT_SERVICE_URL"] = "http://minecraft-service"
      ENV["INTER_SERVICE_API_KEY"] = "test-api-key"
    end

    after do
      ENV["MINECRAFT_SERVICE_URL"] = @original_minecraft_url
