...
RSpec.describe SendPasswordResetEmailConsumer do
  ...
  # SendPasswordResetEmailConsumer вызывает JSON.parse(message.payload) — payload JSON-строка
  ...
  before do
    ...
    allow(UserMailer).to receive(:reset_password).and_return(
      instance_double(ActionMailer::MessageDelivery, deliver_now: true)
    )
  end

  describe '#consume' do
    it 'calls UserMailer.reset_password with correct arguments' do
      consumer.consume

      expect(UserMailer).to have_received(:reset_password).with(email, token, nickname, time_zone)
    end

    it 'delivers the email immediately' do
      mailer_double = instance_double(ActionMailer::MessageDelivery, deliver_now: true)
      allow(UserMailer).to receive(:reset_password).and_return(mailer_double)
    ...
    context 'when time_zone is invalid' do
      ...
      it 'falls back to UTC' do
        ...
        expect(UserMailer).to have_received(:reset_password).with(email, token, nickname, 'UTC')
      end
    end
    ...
    context 'when message processing raises an error' do
      before do
        allow(UserMailer).to receive(:reset_password).and_raise(StandardError.new('delivery failed'))
      end
    ...
    context 'when payload is invalid JSON' do
      ...
      it 'does not call the mailer' do
        ...
        expect(UserMailer).not_to have_received(:reset_password)
      end
    end
    ...
    context 'with multiple messages' do
      let(:message2) do
        instance_double(
          ...
          payload: raw_payload.merge('token' => 'other_reset_token').to_json
        )
      end
      ...
      it 'processes each message' do
        ...
        expect(UserMailer).to have_received(:reset_password).twice
      end
    end
  end
end
