...
class AdminController < ApplicationController
  ...
  def edit_player
    ...
    unless profile_json.present?
      ...
      response = HTTParty.get(
        "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/players/#{nickname}",
        headers: {
    ...
    unless punishments_json.present?
      ...
      response = HTTParty.get(
        "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/players/#{nickname}/punishments",
        headers: {
          "Authorization" => "Bearer #{ENV['INTER_SERVICE_API_KEY']}"
        }
      )
  ...
  def change_password
    ...
    response = HTTParty.post(
      "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/players/#{nickname}/validate_password",
      headers: {
  ...
  def update_account
    ...
    if !changed_pass.nil?
      ...
      if changed_pass
        response = HTTParty.get(
          "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/users/#{user_id}/get_password",
          headers: {
            "Authorization" => "Bearer #{ENV['INTER_SERVICE_API_KEY']}"
          }
        )
  ...
  def removed_players
    ...
    begin
      api_url  = "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/removed_players"
      response = HTTParty.get(
        api_url,
        headers: {
          "Authorization" => "Bearer #{ENV['INTER_SERVICE_API_KEY']}"
  ...
  def add_to_removed_players
    ...
    api_url = "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/removed_players/add/#{nickname}"
    response = HTTParty.post(
      api_url,
      headers: {
        "Accept" => "application/json",
  ...
  def punishment_appeals
    ...
    begin
      ...
      api_url = "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/user/punishment_appeal_all"
  ...
  def get_punishment_appeal
    ...
    api_url = "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/user/punishment_appeal/full/#{id}"
    response = HTTParty.get(
      api_url,
      headers: {
        "Authorization" => "Bearer #{ENV['INTER_SERVICE_API_KEY']}"
  ...
  def punishment_appeal_accept
    id = params[:id]
    api_url = "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/user/punishment_appeal/delete/#{id}"
  ...
  def get_punishment_appeal_data
    ...
    api_url = "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/user/punishment_appeal/get_admin_answer/#{id}"
  ...
  def punishment_appeal_reject
    begin
      ...
      api_url = "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/user/punishment_appeal/reject"
  ...
  def complaints
    ...
    begin
      # Формируем правильный URL API с параметрами
      api_url = "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/admin/complaints"
      # Добавляем параметры запроса
      query_params = {
        search: params[:search],
        sort: sort_key,
