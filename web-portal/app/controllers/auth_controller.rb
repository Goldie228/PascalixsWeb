class AuthController < ApplicationController
  ...
  def discord
    ...
    if params[:callback_url].present?
      redirect_to "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/auth/discord?callback_url=#{URI.encode_www_form_component(params[:callback_url])}"
    else
      redirect_to "#{ENV['IDENTITY_SERVICE_URL']}/api/v1/auth/discord"
    end
  end
