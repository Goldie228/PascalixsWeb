...
function PendingPasswordReset() {
  ...
  // Auto-check for password reset status (polling)
  const checkResetStatus = useCallback(async () => {
    if (!email || verified) return

    setIsChecking(true)
    try {
      const response = await userApi.checkPasswordResetStatus(email)
      if (response.data?.verified) {
        setVerified(true)
        toastSuccess(t('pending_pages.check_reset_success'))
  ...
  const handleResend = async () => {
    ...
    setIsResending(true)
    try {
      // Re-trigger the password reset email
      await userApi.sendResetPasswordEmail({ email })
      toastSuccess(t('pending_pages.resend_reset_success'))
      ...
      const message =
        ...
        t('pending_pages.resend_reset_error')
      toastError(message)
    } finally {
          ...
          <CardContent className="space-y-6">
            ...
            <Alert variant="info">
              ...
              <AlertContent className="text-sm">
                <p>{t('pending_pages.reset_sent_message')}</p>
              </AlertContent>
            </Alert>
            ...
            <div className="rounded-lg bg-base-200 p-4">
              <div className="mb-3 flex items-center gap-2 text-warning">
                ...
                <h4 className="font-medium">
                  {t('pending_pages.reset_steps_title')}
                </h4>
              </div>
              <div className="space-y-2 text-sm text-base-content/80">
                <div className="flex items-start gap-2">
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-warning/10 text-xs font-medium text-warning">
                    1
                  </span>
                  <span>{t('pending_pages.reset_step1')}</span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-warning/10 text-xs font-medium text-warning">
                    2
                  </span>
                  <span>{t('pending_pages.reset_step2')}</span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-warning/10 text-xs font-medium text-warning">
                    3
                  </span>
                  <span>{t('pending_pages.reset_step3')}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-center gap-2 text-sm text-base-content/50">
              {isChecking ? (
                <>
                  ...
                  <span>{t('pending_pages.check_reset_status')}</span>
                </>
              ) : verified ? (
                <>
                  <CheckCircle2 className="h-4 w-4 text-success" />
                  <span className="text-success">
                    {t('pending_pages.check_reset_success')}
                  </span>
                </>
              ) : (
                <>
                  <Loader2 className="h-4 w-4 animate-spin" />
                  <span>
                    {t('pending_pages.check_reset_status')} ({Math.min(autoCheckCount * 5, 60)}s)
                  </span>
                </>
              )}
            </div>
